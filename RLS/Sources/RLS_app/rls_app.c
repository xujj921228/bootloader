...
uint8  u8_G_FW_Config;
uint8  u8_G_IR_Config;
...
uint16 u16_window_cnt;

uint16 u16_IR_left_AD;
uint16 u16_IR_right_AD;


struct BCM_01_Frame         Lin_BCM_WIPSW;
struct BCM_02_Frame         Lin_BCM_MOTOR;
...
void RLS_AutoLightControl(void)
{
    ...
    uint32 temp_Twilight_On_th, temp_Twilight_Off_th;
    ...
    if(u8_Brightness_IR <= 5)
	{
		...
		temp_Twilight_On_th  =  Light_Stastegy_Parameter[0].PositionLamp_on_th;
		temp_Twilight_Off_th =  Light_Stastegy_Parameter[0].PositionLamp_off_th;
	}
	else if(u8_Brightness_IR <= 10)
	{
		...
		temp_Twilight_On_th  =  Light_Stastegy_Parameter[0].PositionLamp_on_th + 16;
		temp_Twilight_Off_th =  Light_Stastegy_Parameter[0].PositionLamp_off_th + 16;
	}
	else 
	{
		...
		temp_Twilight_On_th  =  Light_Stastegy_Parameter[0].PositionLamp_on_th + 33;
		temp_Twilight_Off_th =  Light_Stastegy_Parameter[0].PositionLamp_off_th + 33;
	}
...
void RLS_Get_IR_Process(void)
{
	uint32 temp_IR_left;
	uint32 temp_IR_right;
	
	temp_IR_left  = get_adc_times(LEFT_CH, IR_WINDOW);
	temp_IR_right = get_adc_times(RIGHT_CH, IR_WINDOW);
	
	//temp_IR_left = get_adc(LEFT_CH);
	//temp_IR_right = get_adc(RIGHT_CH);
	
	u16_IR_left_AD  = temp_IR_left;
	u16_IR_right_AD = temp_IR_right;
}
...
uint8 RLS_Get_Rain_State(uint8 PD_chan)
{
    ...
    if((PD_chan == PDA)&&(u16_Pd_Measure_Value!=0)) 
    {
        ...
        if(Lin_BCM_MOTOR.ParkPosition == 1)
        { 
...
/*******************************************************
 * FUNCTION NAME : RLS_Wiper_Ctl()
 *   DESCRIPTION : Wiper control function 
 *         INPUT : NONE
 *        OUTPUT : void  
 *        RETURN : NONE              
 *        OTHERS : NONE
 *******************************************************/
uint8 RLS_Wiper_Ctl(uint8 Int_Timer, uint8 Wiper_Timer)
{
...
void RLS_Rain_State_Mchaine(void)
{
    ...
    switch(u8_Wiper_State)
    {
        ...
        case PERIOD_SPEED_MODE:
        {
            ...
            //速度选择次数
        	if(u16_SPD_Vehicle >= 0 && u16_SPD_Vehicle < 10)
        	{
        		//u8_cnt_choose = 4;
        		switch(u8_Rain_Sensitivity)
        		{
        			case 1:
        			{
        				u8_cnt_choose = 3;
        	...
        	else if(u16_SPD_Vehicle >= 10 && u16_SPD_Vehicle < 80)
        	{
        		switch(u8_Rain_Sensitivity)
				{
					case 1:
        	...
        	else if(u16_SPD_Vehicle >= 80 && u16_SPD_Vehicle < 120)
        	{
        		switch(u8_Rain_Sensitivity)
				{
					case 1:
        	...
        	{
        		u8_Int_Time = 70;
        		temp_period_low_th = Rain_Stastegy_Parameter[0].period_enter_low_th+1 ;
        	...
        	{
        		u8_temp_rcv = RLS_Wiper_Ctl(u8_Int_Time_pre, u8_IntDelayTimer);
        		u8_Wiper_State = PERIOD_SPEED_MODE;
        	}
        	else 
        	{
        	...
        	{
        		u8_ExitLowSpeedTimer++;
        		if(u8_ExitLowSpeedTimer >= 400)
        		{
        			u8_ExitLowSpeedTimer = 0;
        			u8_EnterLowSpeedCnt = 0;
        		}
...
{
    if(Lin_BCM_MOTOR.ParkPosition == 0)   //1为park点
    {
        u8_MeasureSureTime++;
...
void RLS_SelfAdaptTask(void)
{
   ...
   /* add intia*/
    ...
    u8_WiperSpeed = 0 ;
	l_bool_wr_LI0_S_AUTO_H(u8_WiperSpeed);
	l_bool_wr_LI0_S_AUTO_L(u8_WiperSpeed);
	l_bool_wr_LI0_S_AUTO_Panel(0);
	l_bool_wr_LI0_S_Auto_Lamp(0);
/******************Chanl A*********************/   
    for(i = 0; i < 20; i++)
    {
    	WDOG_Feed();
...
{
    uint8 i;
    if(Lin_BCM_WIPSW.SCS_Automatic == 0)
    {
        for(i = 0; i < Rain_WINDOW; ++i) 
        {
            u8_RainIntensity_Win[i] = 0;
    ...
    if((u8_Wipe_Automatic_On_Pre == 0)&&(Lin_BCM_WIPSW.SCS_Automatic == 1))
    {
        u8_Wipe_Automatic_On_Flg = 1;
    } 
    
    u8_Wipe_Automatic_On_Pre = Lin_BCM_WIPSW.SCS_Automatic;  
...
void Sleep_Process(void)
{
	...
	//if((Lin_BCM_WIPSW.Status_SMA >= 1)&&(Lin_BCM_WIPSW.Status_SMA <= 4))
	if(Lin_BCM_WIPSW.Status_SMA == 2)
	{
		RTC_EnableInt();
