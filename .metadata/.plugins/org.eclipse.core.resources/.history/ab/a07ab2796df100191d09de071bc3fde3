...
{
    IIC_SCL_PULLUP ;                    //Turns off the internal pull-up, using the external pull-up PTA
    IIC_SDA_PULLUP ;                    //Turns off the internal pull-up, using the external pull-up PTB
...
uint8 DRV_IIC_READ_BYTE(uint8 AckValue)
{
    ...
    if(AckValue)                     //send local response
	{
    	IIC_SDA_High();	
	}                                
	else
