...
uint8   Temp_error_cnt,Hum_error_cnt,iic_error_cnt;

float  f_Humid_buffer[HUMID_NUM],f_Temp_buffer[TEMP_NUM];
float  f_Humid_Avg,f_Temp_Avg;
		...
		if(DRV_IIC_WRITE_BYTE(SHT30_SACQ_CMD>>8)==ACK)                      //Command
		{
			if(DRV_IIC_WRITE_BYTE(SHT30_SACQ_CMD&0xFF)==ACK)
			{
				...
				if(TempCrc == crc8(Temper,2))
				{
					...
					if(temp >= 8650488)
					{
						temp = 8650488;							//Temp_measure  max value 87℃
					}
					
					if(temp <= 327670)
					{
						temp = 327670;							//Temp_measure  minimum value 0℃
					}
#if 1
					f_acture_temp =  (float)(temp/65534 - 45); //f_acture_temp
#endif						
					Temp_measure = (uint8)((temp*2)/65534 + 80 - 90 );  //0.5*D-39.6 
				...
				if(HumCrc == crc8(Temper,2))                   
				{
					  ...
					  temp = (unsigned long)Hum_measure_TP*100;                    
                          Hum_measure = (uint16)((temp*2)/65534 + 1 );
#if 1
...
/****************************************************************************************************
 * FUNCTION NAME : void Cal_Avg_Function(float temp ,uint8 n)
 *   DESCRIPTION : 浮点均值计算
 *         INPUT : cmd，
 *        OUTPUT : Temp_measure,Hum_measure 
 *        RETURN : uint8                    
 *        OTHERS : NONE
 ****************************************************************************************************/
void Cal_F_Avg_Function(void)
{
	uint8 i ;
	double sum = 0;
	for(i = 0;i < HUMID_NUM;i++)
	{
		sum += f_Humid_buffer[i] ;
	}
	f_Humid_Avg = sum / HUMID_NUM;
...
void  Humid_Avg_Function(void)
{
    ...
    {      
        ...
        f_Humid_buffer[HUMID_NUM - 1] =  f_acture_hum;
        if(i < HUMID_NUM)   f_Humid_buffer[i - 1] = f_Humid_buffer[i] ;
    }
...
void  Temp_Avg_Function(void)
{
    ...
    {      
        ...
        f_Temp_buffer[TEMP_NUM - 1] =  f_acture_temp;
        if(i < TEMP_NUM)   f_Temp_buffer[i - 1] = f_Temp_buffer[i] ;
    }
